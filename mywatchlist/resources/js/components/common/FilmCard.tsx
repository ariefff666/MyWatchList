...
import axios from 'axios'; // Untuk submit rating
...
interface FilmCardProps {
    film: FilmType;
    onCardClick?: () => void;
    onRemoveFromPlaylist?: () => void; // Callback tanpa argumen, ID sudah ada di context FilmCard
    onRateFilm: (newRating: number | null) => void; // Callback untuk update UI di parent
}

const FilmCard: React.FC<FilmCardProps> = ({ film, onCardClick, onRemoveFromPlaylist, onRateFilm }) => {
        ...
        <div className="group flex h-full transform flex-col overflow-hidden rounded-lg bg-gray-700/80 shadow-xl backdrop-blur-sm transition-transform duration-300 hover:scale-[1.03]">
            ...
            <div className="flex flex-grow flex-col p-3 md:p-4">
                ...
                <div className="mt-auto space-y-2 border-t border-gray-600/50 pt-3">
                    {!isRatingMode && onRemoveFromPlaylist && (
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                onRemoveFromPlaylist();
                            }}
                            className="flex w-full items-center justify-center rounded bg-red-600/80 px-2 py-1.5 text-xs text-white transition-colors duration-200 hover:bg-red-600"
                        >
                            <TrashIcon className="mr-1.5 h-3.5 w-3.5" /> Hapus dari Playlist
                        </button>
                    )}
                </div>
            </div>
